...
print("Loading model for Pre-Flight MAX Noun Attention...")
...
# --- TELEMETRY MODE: Set to 0.00 to just log the numbers ---
THRESHOLD = 0.00 
...
for i, line in enumerate(lines):
    ...
    # Extract the target noun from the POPE question
    ...
    # Get Vanilla Answer from logits
    ...
    # Audit the visual evidence for the noun token specifically
    if ans_vanilla == "yes" and target_noun:
        ...
        if target_indices:
            # 2. Extract Anchor Head Attention from the last layer
            attn_matrix = outputs.attentions[-1][0] 
            ...
            # If the noun is split into multiple tokens, take the max mass across those subwords
            noun_attn_mass = max(masses)
            print(f"  -> [DEBUG] Target: '{target_noun:<12}' | Pre-Flight MAX Attn Mass: {noun_attn_mass:.4f}")
...
print("\n=== PRE-FLIGHT MAX ATTENTION RESULTS ===")
