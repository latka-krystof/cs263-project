...
class AdaptiveAttentionLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if attn_matrix is not None:
            ...
            # --- THE MEAN MASS CALCULATION ---
            # Sum the total visual attention mass of the averaged anchor heads
            mean_mass = visual_attn.sum().item()
            
            # --- TELEMETRY ---
            top_token_id = torch.argmax(scores[0]).item()
            if top_token_id in self.target_tokens:
                print(f"  -> [DEBUG] 'Yes' triggered. MEAN MASS: {mean_mass:.4f}")
            
            # Note: For mass, we penalize LOW numbers (meaning the model isn't looking at the image)
            # But we leave threshold at 0.00 for now to passively gather telemetry data.
            if mean_mass < self.threshold and self.threshold > 0.00: 
...
# Threshold set to 0.00 to allow baseline logging to flow through unimpeded
adaptive_processor = AdaptiveAttentionLogitsProcessor(monitor, processor, threshold=0.00)
